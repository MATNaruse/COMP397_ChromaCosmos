        ...
        public GenerateRandomWaves(numOfWaves: number, aliensPerWave:number = 5, basic:boolean = true){
            ...
            for(var i = 0; i < waves; i++){
                ...
                for(var j = 0; j < perWave; j++){
                    ...
                    // Generating Alien
                    var new_alien;
                    if(colourPicked < 3) new_alien = new objects.PrimaryAlien(managers.Game.assetManager, colourPicked, this.LaneXValues[currentLane], yWaveOffset);
                    else new_alien = new objects.SecondaryAlien(managers.Game.assetManager, colourPicked, this.LaneXValues[currentLane], yWaveOffset);
        ...
        public GenerateStaticWaves(template: Array<number[]>){
            ...
            template.forEach(row => {
                ...
                row.forEach(num => {
                    ...
                    switch(num){
                        case(0):
                            new_alien = new objects.PrimaryAlien(managers.Game.assetManager, 0, this.LaneXValues[currLane], yWaveOffset);
                            break;
                        case(1):
                            new_alien = new objects.PrimaryAlien(managers.Game.assetManager, 1, this.LaneXValues[currLane], yWaveOffset);
                            break;
                        case(2):
                            new_alien = new objects.PrimaryAlien(managers.Game.assetManager, 2, this.LaneXValues[currLane], yWaveOffset);
                            break;
                        case(3):
                            new_alien = new objects.SecondaryAlien(managers.Game.assetManager, 3, this.LaneXValues[currLane], yWaveOffset);
                            break;
                        case(4):
                            new_alien = new objects.SecondaryAlien(managers.Game.assetManager, 4, this.LaneXValues[currLane], yWaveOffset);
                            break;
                        case(5):
                            new_alien = new objects.SecondaryAlien(managers.Game.assetManager, 5, this.LaneXValues[currLane], yWaveOffset);
                            break;
                        case(-1):
                            new_alien = null;
                            break;
